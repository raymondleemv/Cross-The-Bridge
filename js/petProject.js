function boatSpace() {
	let boatPassengers = $('#boat-container').find('.passenger-button');
...
function reset() {
	$('#game-content').removeClass('unclickable');
	$('#boat-container').find('.passenger-button').hide();
	$('.passengers-container.crossed-bridge').find('.passenger-button').hide();
...
$(document).ready(function () {
	...
	let onboardPassengers = $('#boat-container').find('.passenger-button');
	let crossedBridgePassengers = $('.passengers-container.crossed-bridge').find(
		'.passenger-button'
	);
	let passengers = $('.passengers-container')
		.not('.crossed-bridge')
		.find('.passenger-button');
	...
	$('.passenger-button').click(function () {
		...
		if ($(this).parent().is('#boat-container')) {
			if ($('#boat-container').hasClass('cross-bridge')) {
				$(
					crossedBridgePassengers
						.parent()
						.find('[data-person=' + $(this).attr('data-person') + ']')
						.filter(hidden(true))[0]
				).show();
			} else {
				$(
					passengers
		...
		if (boatPassenger !== null) {
			...
			boatPassenger
				.find('.passenger-icon')
				.attr('src', $(this).find('.passenger-icon').attr('src'));
			boatPassenger
				.find('.passenger-icon')
				.attr(
					'alt',
					'This is the ' + $(this).attr('data-person') + 'on the boat button'
				);
			boatPassenger.show();
		}
	});
	...
	$('#move-boat-btn').click(function () {
		...
		if (onboardPassengers.filter(hidden(true)).length > 1) {
			$('#error-message')
				.text('Oops, it seems that no one is on the boat to drive!')
				.show();
			console.log('hi');
		...
		let numPolicemanLeft = passengers
			.parent()
			.find('[data-person=policeman]')
			.filter(hidden(false)).length;
		let numCriminalLeft = passengers
			.parent()
			.find('[data-person=criminal]')
			.filter(hidden(false)).length;
		let numPolicemanRight = crossedBridgePassengers
			.parent()
			.find('[data-person=policeman]')
			.filter(hidden(false)).length;
		let numCriminalRight = crossedBridgePassengers
			.parent()
			.find('[data-person=criminal]')
			.filter(hidden(false)).length;

		if (!$('#boat-container').hasClass('cross-bridge')) {
			numPolicemanRight += onboardPassengers
				.parent()
				.find('[data-person=policeman]').length;
			numCriminalRight += onboardPassengers
				.parent()
				.find('[data-person=criminal]').length;
		} else {
			numPolicemanLeft += onboardPassengers
				.parent()
				.find('[data-person=policeman]').length;
			numCriminalLeft += onboardPassengers
				.parent()
				.find('[data-person=criminal]').length;
		}
		if (numPolicemanLeft !== 0 && numCriminalLeft > numPolicemanLeft) {
			$('#error-message')
				.text('Oops, there are more criminals than policemen on the left!')
				.show();
			$('#error-message').focus();
			return;
		}
		if (numPolicemanRight !== 0 && numCriminalRight > numPolicemanRight) {
			$('#error-message')
				.text('Oops, there are more criminals than policemen on the right!')
				.show();
			$('#error-message').focus();
			// console.log('hi');
			// console.log(document.getElementById('error-message'));
	...
	$('#instructions-btn').click(function () {
		...
		$('#game-buttons')
			.children(':not(#instructions-btn)')
			.toggleClass('unclickable');
	});
	...
	$('#solution-btn').click(async function () {
		// 1. Reset game
		$('#boat-container').hasClass('cross-bridge')
			? await waitForClick($('#play-again-btn'), 2000)
			: reset();
